export const GAME_EVENTS = {
    ATTACK: 'attack',
    END_TURN: 'end-turn',
    ATTACK_REJECTED: 'attack-rejected',
    CARD_ATTACK_READY: 'card-attack-ready',
    TURN_STATE: 'turn-state',
    REROLL_BEGIN: 'reroll-begin',
    REROLL_CONFIRM: 'reroll-confirm',
    REROLL_CANCEL: 'reroll-cancel',
    REROLL_STATE: 'reroll-state',
    CHAIN_START_STATE: 'chain-start-state',
    CHAIN_PATH_LIT: 'chain-path-lit',
    SCENE_READY: 'current-scene-ready',
    START_BATTLE: 'start-battle',
    START_PUZZLE: 'start-puzzle',
    TUTORIAL_WIZARD_APPLY_PHASE: 'tutorial-wizard-apply-phase',
    TUTORIAL_WIZARD_ATTACK: 'tutorial-wizard-attack',
    TUTORIAL_WIZARD_ENERGY_DEPLETED: 'tutorial-wizard-energy-depleted',
    TUTORIAL_WIZARD_COMPLETE: 'tutorial-wizard-complete',
    TUTORIAL_WIZARD_LAYOUT: 'tutorial-wizard-layout',
    TUTORIAL_WIZARD_REQUEST_LAYOUT: 'tutorial-wizard-request-layout',
    PUZZLE_STATE: 'puzzle-state',
    PUZZLE_RESOLVED: 'puzzle-resolved',
    BATTLE_WON: 'battle-won',
    BATTLE_LOST: 'battle-lost',
    RUN_ATTACK_COUNT: 'run-attack-count',
    PILE_VIEW_OPEN: 'pile-view-open',
    UI_OVERLAY_ACTIVE: 'ui-overlay-active',
    RUN_PHASE: 'run-phase',
    COMBAT_RECAP: 'combat-recap',
    PHASE_SHIFT: 'phase-shift',
    PLAY_SFX: 'play-sfx',
    SET_BGM: 'set-bgm',
} as const;
