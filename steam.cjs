 ...
 *
 * Steam is optional and not required to run desktop builds. Init only runs when an
 * App ID is present (STEAM_APP_ID or steam_appid.txt). Otherwise this module is a no-op
 * so packaged apps open normally without Steam.
 */
...
/**
 * Call before `app.whenReady()` so the Steam overlay can hook in when Steam is enabled.
 * Without an App ID this is intentionally a no-op (desktop builds are not Steam-gated).
 */
const bootstrap = () =>
{
    ...
    // Relaunch-via-Steam is opt-in; default off so local/packaged builds still open.
    if (process.env.SIGNAL_CHAIN_STEAM_RELAUNCH === '1'
        && steamworks.restartAppIfNecessary?.(appId))
    {
        process.exit(0);
        return;
    }
    ...
    try
    {
        ...
        console.info('[steam] initialized', appId);
    }
    catch (error)
    {
