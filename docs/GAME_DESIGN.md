...
| Battle modifiers | `battleModifiers.ts`, `battle-mod` behavior, `battle-mod` enemy intent | ±10% to enemy attack, damage taken, shield gained, or damage dealt — player cards (Glitch/Hardwire/Patch/Overclock) and enemy intents. **All modifiers last until energy refills.** Active buffs/debuffs show as icons above the player panel (`BattleModifierStatusView`). |
...
| **Curse cards** | `cards.json` (`unplayable`, `handEndPenalty`), `CardGameSession.resolveHandEndPenalties` | Bad cards that clog the hand — **Burden** (unplayable, 5 dmg if held at end of turn then exhausted), **Fuse** (weak attack, 8 dmg if not placed by end of turn then exhausted). Penalties resolve after **each attack** (each enemy response), not only when energy depletes. **Saboteur** adds Burdens via `curseHand` passive |
