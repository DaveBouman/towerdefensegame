...
| 2026-07-14 | **Battle modifiers (±10%).** New `battle-mod` system (`battleModifiers.ts`): four stats — enemy attack, damage taken, shield gained, damage dealt — stack in ±10% steps. Player cards: **Glitch** (-enemy atk), **Hardwire** (+shield), **Patch** (-damage taken), **Overclock** (+damage dealt). Enemy intents can telegraph the same modifiers before attack/traps. Modifiers from chain cards last through the enemy response; cleared after each enemy turn. |
