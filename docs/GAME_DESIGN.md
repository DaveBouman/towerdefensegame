...
| 2026-07-14 | **Per-attack enemy + persistent board.** Enemy responds after every attack; the board persists until all energy is spent, then clears with the graveyard animation. Mid-round enemy responses refill the hand (`refreshPlayerAfterMidRoundEnemy`); round end calls `finishPlayerRound` after `clearBoard`. |
